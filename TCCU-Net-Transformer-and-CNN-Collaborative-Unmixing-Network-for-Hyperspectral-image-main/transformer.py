...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, d_head, expansion, dropout=0.):# dim_head=d_head   mlp_dim=expansion
        super().__init__()
        self.norm = nn.LayerNorm(dim)
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                ...
                PreNorm(dim, FeedForward(dim, expansion, dropout=dropout))
            ]))
...
class ViT(nn.Module):
    def __init__(self, *, image_size, patch_size, dim, depth, heads, expansion,
                 pool='cls', channels=3, d_head=64, dropout=0., emb_dropout=0.):#dim_head=64
        super().__init__()
        ...
        self.transformer = Transformer(dim, depth, heads, d_head, expansion, dropout)#dim_head
